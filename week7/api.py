...
async def home_page(request: Request):
    return templates.TemplateResponse(
        name = "home.html", 
        context = {"request": request, "title": "歡迎光臨，請註冊登入系統", "signup":"註冊帳號","signin": "登入系統"})
...
def get_member(request: Request):
    ...
    if request.session.get("SIGNED-IN") is True:
        ...
        return templates.TemplateResponse("member.html", {"request": request, "title": "歡迎光臨，這是會員頁", "username": name, "messages": messages})
    else:
        return RedirectResponse('/')   
...
async def update_member(request: Request, response_body = Body("name")):
    ...
    if request.session.get("SIGNED-IN") is True:
        ...
        val = (response_body["name"], username )
        cursor.execute(sql, val)
        mydb.commit()
        print("name in db updated.")
